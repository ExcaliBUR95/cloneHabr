...
const jsonv3Exporter = {
  ...
  stringify: ({
    ...
    return file.whitespacesBefore + stringify__default['default'](file.content, {
...
function exportTranslationKeys(keys, locale, config, cache) {
  ...
  for (const [filePath, keysForFilepath] of Object.entries(keysPerFilepath)) {
    ...
    cache.originalTranslationFiles[filePath] = deepmerge__default['default']((_cache$originalTransl = cache.originalTranslationFiles[filePath]) !== null && _cache$originalTransl !== void 0 ? _cache$originalTransl : {}, loadTranslationFile(exporter, config, filePath), {
    ...
    const directoryPath = path__default['default'].dirname(filePath);
    fs__default['default'].mkdirSync(directoryPath, {
      recursive: true
    });
    fs__default['default'].writeFileSync(filePath, exporter.stringify({
      config,
      file: translationFile
    }), {
      encoding: 'utf8'
...
function computeDerivedKeys(extractedKey, locale, config) {
  ...
  if (parsedOptions.hasCount) {
    ...
    const pluralRule = i18next__default['default'].services.pluralResolver.getRule(locale);
